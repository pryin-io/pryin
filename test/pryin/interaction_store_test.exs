defmodule PryIn.InteractionStoreTest do
  ...
  describe "start_interaction" do
    test "adds a running interaction" do
      ...
      assert %{^pid => %{interaction: ^interaction}} =
               InteractionStore.get_state().running_interactions
    end
    ...
    test "limits number of interactions" do
      ...
      interaction_1 =
        Factory.build(:request, start_time: 1000, duration: 1, controller: "SomeController")

      pid_1 = spawn(fn -> :timer.sleep(5000) end)

      interaction_2 =
        Factory.build(:request, start_time: 1000, duration: 2, controller: "SomeController")

      pid_2 = spawn(fn -> :timer.sleep(5000) end)

      interaction_3 =
        Factory.build(:request, start_time: 1000, duration: 3, controller: "SomeController")
  ...
  test "set_interaction_data" do
    ...
    assert %{^pid => %{interaction: ^new_interaction}} =
             InteractionStore.get_state().running_interactions
  end
  ...
  test "add_ecto_query" do
    ...
    assert InteractionStore.get_interaction(self()).ecto_queries == [
             Interaction.EctoQuery.new(duration: 123)
           ]
  end
  ...
  test "add_view_rendering" do
    ...
    assert InteractionStore.get_interaction(self()).view_renderings == [
             Interaction.ViewRendering.new(duration: 123)
           ]
  end
  ...
  test "add_custom_metric" do
    ...
    assert InteractionStore.get_interaction(self()).custom_metrics == [
             Interaction.CustomMetric.new(duration: 123)
           ]
  end
  ...
  test "finish_interaction with controller and action" do
    ...
    InteractionStore.set_interaction_data(self(), %{
  ...
  defp do_wait_until_process_stopped(pid, timer_ref) do
    receive do
      :stop_waiting_until_process_stopped ->
        raise "timeout waiting for process #{inspect(pid)} to stop"
    after
      0 ->
