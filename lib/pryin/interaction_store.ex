defmodule PryIn.InteractionStore do
  ...
  def handle_cast({:start_interaction, pid, interaction}, state) do
    ...
    if stored_interactions_count(state) >= max_interactions() do
      ...
      running_interaction = %RunningInteraction{
        type: :parent,
  ...
  def handle_cast({:set_interaction_data, pid, data}, state) do
    state =
      update_in(state.running_interactions[parent_pid(state, pid)].interaction, fn interaction ->
        Map.merge(interaction, data)
      end)
  ...
  def handle_cast({:add_ecto_query, pid, data}, state) do
    state =
      update_in(state.running_interactions[parent_pid(state, pid)].interaction, fn
        nil ->
          nil
  ...
  def handle_cast({:finish_interaction, pid}, state) do
    ...
    running_interactions =
      clear_children(running_interactions, finished_running_interaction.child_pids)

    finished_interaction =
      Map.update!(finished_running_interaction.interaction, :start_time, &trunc(&1 / 1000))
    ...
    if forward_interaction?(finished_interaction) do
      ...
      Logger.debug(
        "[PryIn] dropped interaction without controller, action and custom key: #{
          finished_interaction.interaction_id
        }"
      )
  ...
  def handle_cast({:add_child, parent_pid, child_pid}, state) do
    state =
      if state.running_interactions[parent_pid] do
        case state.running_interactions[child_pid] do
          nil ->
            add_new_child(state, parent_pid, child_pid)

          %RunningInteraction{type: :parent} when child_pid == parent_pid ->
            state

          %RunningInteraction{type: :parent} ->
            Logger.warn(
              "[PryIn] cannot join #{inspect(child_pid)} to a different join, because it is already running a trace"
            )

            state

          _running_interaction ->
            move_child_to_new_parent(state, parent_pid, child_pid)
        end
      else
  ...
  def handle_cast({:put_context, pid, key, value}, state) do
    state =
      update_in(state.running_interactions[parent_pid(state, pid)].interaction, fn
        ...
        interaction ->
          %{interaction | context: [{to_string(key), to_string(value)} | interaction.context]}
      end)
  ...
  defp stored_interactions_count(%{
         finished_interactions: finished_interactions,
         running_interactions: running_interactions
       }) do
    Enum.count(running_interactions, fn {_pid, interaction} -> interaction.type == :parent end) +
      length(finished_interactions)
  end
  ...
  defp drop_running_interaction(pid, state) do
    ...
    running_interactions =
      case down_running_interaction do
        nil ->
          running_interactions
        ...
        %RunningInteraction{type: :child} ->
          ...
          update_in(
            running_interactions[down_running_interaction.parent_pid].child_pids,
            fn child_pids ->
  ...
  defp add_new_child(state, parent_pid, child_pid) do
    ...
    running_interactions =
      update_in(running_interactions[parent_pid].child_pids, &MapSet.put(&1, child_pid))
  ...
  defp move_child_to_new_parent(state, parent_pid, child_pid) do
    ...
    state =
      update_in(
        state.running_interactions[child_interaction.parent_pid].child_pids,
        fn child_pids ->
    ...
    state =
      update_in(state.running_interactions[parent_pid].child_pids, &MapSet.put(&1, child_pid))
